...
class PDFProcessor:
    ...
    def _initialize_qdrant_collection(self):
        ...
        try:
            ...
            # collection may already exist; just note it
    ...
    def extract_text(self, content: bytes, filetype: str) -> str:
        ...
        try:
            if filetype in ('bin', 'zip', 'large_file'):
                return ""
    ...
    async def refine_with_deepseek(self) -> Dict:
        """Use DeepSeek (LLM) to generate answers for all questions in parallel batches"""
        if not self.retrieved_answers:
            return {"answers": []}

        # Check if we have a bin/zip/large_file case
        if not self.chunks or (hasattr(self, 'filetype') and self.filetype in ('bin', 'zip', 'large_file')):
    ...
    async def _process_batch(self, batch: List[Dict]) -> Dict:
        ...
        static_prompt_template = """
            ...
            8.In the case the document has some instructions to solve the question(e.g. do a GET request to get city and based on city do another get request) follow all steps mentioned in document and return the answer
            Output Format:
    ...
    def http_get(url: str) -> str:
        """Make an HTTP GET request and return text."""
        r = requests.get(url)
        return r.text
    ...
    async def _get_flight_number_from_finalround(self, document_url: str) -> Dict:
        ...
        but WITHOUT hardcoding city-landmark mapping or endpoints.
        The LLM will read the PDF context and perform the steps itself.
        """
        try:
            # 1. Download the PDF
            file_content, self.filetype = self.download_file(document_url)
            if self.filetype != 'pdf':
                return {"answers": ["Document is not a PDF"]}

            # 2. Extract all text from the PDF
            pdf_text = self.extract_text(file_content, 'pdf')

            # 3. Prepare LangChain LLM + tools
            ...
            # 4. Create the instruction for the LLM
            prompt = f"""
You are given the contents of a document that contains rules for obtaining a flight number.

DOCUMENT CONTENT:
{pdf_text}

TASK:
Determine the flight number by strictly following the instructions in the document.
You may use the http_get tool to make GET requests to fetch the favorite city and to query the correct API endpoint.
Return ONLY the final flight number as your answer, no explanations.
"""

            # 5. Run the agent
            flight_number = agent.run(prompt).strip()

            return {"Flight Number": [flight_number]}

        except Exception as e:
            print(f"Error in finalround agent: {e}")
            return {"answers": ["Error processing flight number request"]}

    async def _secret_token_agent(self, document_url: str) -> Dict:
        """
        Agentic secret-token handler:
         - Download page (we do GET)
         - Ask LLM to extract token(s) from HTML and return ONLY a JSON list of strings
        """
        try:
            r = requests.get(document_url, timeout=15)
            r.raise_for_status()
            html = r.text
            prompt = f"""
You are given HTML content (below). Extract the secret token requested by the user.
Output ONLY a JSON array of strings.
Look for token in: <div id='token'>, <span id='token'>, any 'token' keys, or any token-like strings.
...
"""
            ...
            # Fallback: look for token div ourselves then fallback to Could not find
            soup = BeautifulSoup(html, 'html.parser')
            token_div = soup.find('div', {'id':'token'}) or soup.find('span', {'id':'token'})
            if token_div:
                return {"answers":[f"Token: {token_div.text.strip()}"]}
            return {"answers": ["Could not find token in response"]}
        except Exception as e:
            print(f"Error in secret token agent: {e}")
            return {"answers": ["Error fetching secret token"]}

    def process_document(self, document_url: str):
        """Process document through the full pipeline (synchronous helper)"""
        try:
            file_content, self.filetype = self.download_file(document_url)
            
            if self.filetype in ('bin', 'zip', 'large_file'):
                self.chunks = []
                self.document_embeddings = None
                return
    ...
    async def process(self, document_url: str, questions: List[str]) -> Dict:
        ...
        For FinalRound flight questions and secret-token URLs, we use agentic flows.
        """
        try:
            if not document_url or not questions:
                return {"answers": ["Invalid input"]}

            # detect special non-cached cases
            is_secret_token_url = document_url.startswith("https://register.hackrx.in/utils/get-secret-token?")
            is_finalround_flight_query = (
                "FinalRound4SubmissionPDF.pdf" in document_url
                and len(questions) == 1
                and "flight" in questions[0].lower()
            )

            # SECRET TOKEN: use agent that extracts from live HTML via LLM
            if is_secret_token_url:
                return await self._secret_token_agent(document_url)

            # FINAL ROUND FLIGHT: use agent that reads doc and performs actions
            if is_finalround_flight_query:
                result = await self._get_flight_number_from_finalround(document_url)
                # Explicitly don't cache this result
                return result
            ...
            # check cache for document (if exists)
            if document_url in self.url_to_collection:
                ...
                for idx, question in enumerate(questions):
                    coll_key = str(self.url_to_collection[document_url])
                    if coll_key in self.qa_cache and question in self.qa_cache[coll_key]:
                        cached_answers.append({
                            'index': idx,
                            'answer': self.qa_cache[coll_key][question]
                        })
                    else:
                ...
                # compute embeddings for new questions, search, and call LLM for them
                ...
                coll_key = str(self.url_to_collection[document_url])
                if coll_key not in self.qa_cache:
                    self.qa_cache[coll_key] = {}

                for question, answer in zip(new_questions, new_answers):
                    self.qa_cache[coll_key][question] = answer
                self._save_qa_cache()
            ...
            # (re)initialize collection (will use self.collection_name) and store vectors
            self._initialize_qdrant_collection()
            self.store_embeddings_in_qdrant()

            # questions -> embeddings -> search -> LLM
            self.process_questions(questions)
            self.search_questions(questions)
            result = await self.refine_with_deepseek()

            # cache QA for this collection
